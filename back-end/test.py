...
* start with greetings instead of directly dive into the interview
...
"""
    ...
    def generate_interview_evaluation(self,Interview_data):
        evaluation_prompt="""
...
"""
        ...
        evaluation_response = self.conversation.invoke({"text": evaluation_prompt})
...
async def upload_resume(resume_file: UploadFile = File(...), job_description: str = Form(...), difficulty_level: str = Form(...)):
    resume_text = processor.extract_text_from_pdf(resume_file.filename)
    processor.create_system_prompt(resume_text, job_description, difficulty_level)
    ai_start = "Welcome! Let's begin the interview. Please tell me about yourself."
...
async def user_input(user_message: str = Form(...)):
    response= processor.process_user_message(user_message) 
...
async def end_interview(payload: ConversationPayload):
    ...
    evaluation = processor.generate_interview_evaluation(interview_data)
