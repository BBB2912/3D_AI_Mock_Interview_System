...
const InterviewPanel = ({ setEvaluation }) => {
  ...
  const evaluationResult = useLoader(THREE.FileLoader, "results.json");
  ...
  const getConversation = () => {
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      if (botResponses[i] && botResponses[i].Interviewer !== undefined) {
        mergedConversation.push({ sender: "Interviewer", text: botResponses[i].Interviewer });
      }
  ...
  const showEvaluationPallet = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/end_interview/", {
        method: "POST",
        headers: {
      ...
      fileReader.onload = () => {
        try {
          ...
          console.error("Error parsing JSON file from blob:", error);
           MySwal.fire({ // Show error if JSON parsing fails
              icon: "error",
  ...
  const setAnimationTrigger = (response) => {
  ...
  const updateConversation = () => {
    ...
    try {
        const userResponses = JSON.parse(sessionStorage.getItem("userResponses") || "[]");
    ...
    <div className="container">
      <div className="container-media">
        ...
        <div className="right-side">
          <Me refresh={updateConversation} setAnimation={setAnimationTrigger} getEvaluation={showEvaluationPallet} />
        </div>
      </div>

      {showButton && (
        <button onClick={showEvaluationPallet} disabled={loading} className="Interview-button">
          {loading ? <FontAwesomeIcon icon={faRotate} spin /> : "Generate Results"}
        </button>
      )}
